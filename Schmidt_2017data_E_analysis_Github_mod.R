...
setwd("~/Documents/josh_r_analysis/fst/Drosophila_ICP_Rudman2017")
library(ggfortify)
library(ggpubr)
library(matrixStats)
library(stats)
...
afmat.17.E.F <- read.table("afmat.17.E.F.txt", header=TRUE) #allele frequency table
names(afmat.17.E.F)  <- gsub("[.]","-",names(afmat.17.E.F) ) #modify sample names
samps.17.E.F <- read.table("samps.17.E.F.txt", header=TRUE) #metadata table
sites.17 <- read.table("sites.17.txt", header=TRUE) #loci
...
afmat.17.E <- afmat.17.E.F[,samps.17.E.F$treatment == "E"]
dim(afmat.17.E)
dim(samps.17.E.F[samps.17.E.F$treatment == "E",])
samps.17.E.F[samps.17.E.F$treatment == "E",]$sampID == cbind.data.frame(colnames(afmat.17.E))
samps.17.E <- transform(samps.17.E.F[samps.17.E.F$treatment == "E",],rep=as.numeric(factor(cage)))
...
# Make plots
pca_res.founder <- prcomp(t(afmat.17.E.F), scale. = TRUE)
#Remove tpt1 due to noise from combined sequencing runs
pca_res.founder.filt <- prcomp(t(afmat.17.E.F[,which(samps.17.E.F$tpt != 1)]), scale. = TRUE)
...
glmmanh4 <- ggplot(manh.snp.cand2, aes(bp, logp4)) + geom_point(colour = "grey") +
  ...
  scale_color_manual(values=c("black","#56B4E9","#D81B60","#FFC107")) +
  facet_grid(~ chrom, scales = "free_x", space = "free_x") +
  scale_x_continuous(n.breaks = 4,guide = guide_axis(angle = 45)) + 
  labs(col="candidate\nSNP\ncategory") +
  xlab("chromosome position") +
...
#Put them together
png("/Users/rudmanlab/Documents/josh_r_analysis/fst/E.2017.glm.cand.SNPs.not1.png",
    width = 1000, height = 750)
...
samps.17.E.F.mod <- transform(samps.17.E.F, rep=as.numeric(factor(cage)))
samps.17.E.F.mod[1,3] <- "E"
samps.17.E.F.mod[2,3] <- "E"
#Combine allele frequency stats with metadata 
af.plot <- cbind(t(data.frame(afmat.17.E.F[581837,])),samps.17.E.F.mod,alpha="0.35")[,c(5,4,6,1,9)]
names(af.plot)[4] <- "af"
#summarize allele frequencies by timepoint
af.plot.mean <- af.plot %>%
  group_by(treatment, tpt) %>% 
  summarise_at(vars("af"), mean)
af.plot.mean[af.plot.mean$tpt=="founder",2] <- "0"
af.plot.mean <- as.data.frame(cbind(cage=c("emean","emean","emean","emean","emean"),af.plot.mean,alpha="1"))
ggplot(af.plot.mean,aes(x=as.numeric(tpt),y=af)) + geom_line(alpha = as.numeric(af.plot.mean$alpha)) + theme_classic2()
...
af.plot2[af.plot2$tpt=="1" & af.plot2$alpha==0.35,]$af <- af.plot.mean[5,4] 
